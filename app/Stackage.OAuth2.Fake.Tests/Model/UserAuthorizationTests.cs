...
public class UserAuthorizationTests
{
   ...
   public void scope_properties_equal_the_expected_values(ScopeTestCase testCase)
   {
      var testSubject = CreateAuthorization(
         scope: (Scope)testCase.GivenScope);

      Assert.That((string)testSubject.Scope, Is.EqualTo(testCase.ExpectedScope));
   ...
   private static UserAuthorization CreateAuthorization(Scope scope)
   {
