...
public class create_with_duplicate_refresh_token
{
   ...
   public async Task setup_before_all_tests()
   {
      ...
      var firstHttpResponse = await httpClient.PostAsync(".internal/refresh-token", content);
   ...
   public async Task response_content_should_contain_error_description()
   {
      ...
      Assert.That(errorResponse.ErrorDescription, Is.EqualTo("The given refreshToken already exists"));
   }
}
